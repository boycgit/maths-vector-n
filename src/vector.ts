...
import { invariant, fail, isPlainObject, hasProxy } from './util';

/**
 * Vector - dim-N vector class for common vector operations, support [big.js](https://github.com/MikeMcl/big.js) for arbitrary-precision decimal arithmetic
 */
...
export interface IVector<T> {
  [propName: string]: Operand<T>;
}

// export type VectorOperand<T> = Vector<T> | Operand<T>;

const REG_AXIS_NAME = /^x[1-9]\d{0,}$/;

/**
 * Vector - n-dimension vector class for common vector operations, support [big.js](https://github.com/MikeMcl/big.js) for arbitrary-precision decimal arithmetic
 */
class BaseVector implements ISpace {
  ...
  values: Array<any>;
  // 每个实例对应的操作体系
  operatorSystem = BaseVector.SYSTEM;
   ...
   * @param dim
   * @example
   * var v = new Vector3({1:3, 2:5, 7:4}, 9); // 稀疏矩阵
   * var v = new Vector3([1,2,4], 3); // 数组转换成
   */
  constructor(dim, xs) {
    this._dim = dim;
    this.values = [];
    if (Array.isArray(xs)) {
      this.values = xs.slice(0, dim);
    } else if (isPlainObject(xs)) {
      Object.keys(xs).forEach(key => {
        const keyNumber = parseInt(key);
        if (keyNumber <= this.dim && parseInt(key) > 0) {
          this.values[keyNumber - 1] = xs[key];
        }
      });
    }
  }
  ...
  set dim(val) {
    fail('can not reset dim of space');
  }

  config({ system = BigOperatorSystem }) {
    if (typeof system === 'string') {
      this.operatorSystem = OperatorSystem[system] || BaseVector.SYSTEM;
    } else {
   ...
   */
  x(index) {
    invariant(index <= this.dim, `index should not more than dim:${this.dim}`);
    invariant(index >= 1, `index should not less than 1`);
    return (this.values[index - 1] || 0).toString();
  }

  static fromArray(dim, target, arr) {
    return new target(dim, arr);
  }

  static fromObject(dim, target, obj) {
    return new target(dim, obj);
  }
}

const ProxyBaseVector = new Proxy(BaseVector, {
  construct: function(target: typeof BaseVector, args: any) {
    ...
    return new Proxy(instance, {
      get: function(obj, prop: string) {
        ...
        } else if (REG_AXIS_NAME.test(prop)) {
...
// console.log('nnnn', Object.keys(ProxyBaseVector));

export default function createSpace(dim: number) {
  invariant(dim > 1, 'space dim should not less than 2');

  let VectorN = function(xs) {
    if (hasProxy) {
      // 使用 proxy 特性，方便程序调用
      return new ProxyBaseVector(dim, xs);
    } else {
      return new BaseVector(dim, xs);
    }
  };
  // 将 BaseVector 类的 prototype、constructor 复制到 VectorN 中
  VectorN.prototype = BaseVector.prototype;
  VectorN.constructor = BaseVector.constructor;
  Object.keys(BaseVector).forEach(name => {
    const target = hasProxy ? ProxyBaseVector : BaseVector;
    VectorN[name] =
      typeof target[name] === 'function'
        ? function(...params) {
            return target[name](dim, target, ...params);
          }
        : BaseVector[name];
  });
